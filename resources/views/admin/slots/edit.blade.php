        ...
        <div class="row">
            ...
            <div class="col-md-6">
                ...
                <label>Tên Phim</label><br>
                    <select style="width:400px; height: 30px" disabled>
                        @foreach ($movies as $movie)
                        <option value="{{ $movie->mv_id }}"  {{ $slot->mv_id == $movie->mv_id ? 'selected="selected"' : '' }}> {{ $movie->mv_name }}  |  Bắt Đầu: {{ $movie->mv_start }}  |  Kết thúc : {{ $movie->mv_end }}
                        </option>
                        @endforeach
                    </select>
