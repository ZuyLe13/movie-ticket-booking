...
class MovieController extends Controller
{
    ...
    public function store(Request $request){
        $this->validate($request, [
            ...
            'name' => 'required',
            'start' => 'required|date|after or equal:today',
    ...
    public function show(Movie $movie){
        if ((Slot::where('mv_id',$movie->mv_id)->first()) != null)
        {
            Session::flash('error', 'Phim hiện đang có suất chiếu. Không thể sửa phim !');
            return redirect()->back();
        }
    ...
    public function destroy(Request $request) : JsonResponse{
        ...
        if ((Slot::where('mv_id',$request->input('id'))->first()) != null)
        {
            return response()->json([
                'error' => true,
                'message' => 'Phim được chọn hiện có suất chiếu. Không thể xóa phim !'
            ]);
        }
